...
public class DanceParty {
	...
	public final DanceType[] danceTypes = new DanceType[] {
			new DanceType(0, "Waltz"), new DanceType(1, "Tango"),
			new DanceType(2, "Foxtrot"), new DanceType(3, "Quickstep"),
			new DanceType(4, "Rumba"), new DanceType(5, "Samba"),
			new DanceType(6, "ChaCha"), new DanceType(7, "Jive") };
	...
	public static void main(String[] args) {
		...
		System.out.println("Tardo: " + (end - start) + "ms.");
		System.exit(0);
	}
		...
		if (DanceParty.isDebug) {
			for (int i = 0; i < this.leaders.length; i++) {
				...
				for (int j = 0; j < leader.danceConfirmed.length; j++) {
					...
					if (followerId < 0) {
						System.out.println(String.format("%15s with %3s",
								this.danceTypes[j].name, "--"));
					} else {
						System.out.println(String.format("%15s with %3s",
								this.danceTypes[j].name, followerId));
					}
				}
				System.out.println();
			}
